...
const invite_user = args => {
  ...
  let invite = db(TABLES.SHARE_INVITE)
    .first('invite_id')
  ...
  let promiseArray = [table_invite, request, invite];
  if (args.category === 'gmail') {
    promiseArray.push(
      db('user_socials').update('invite_google', 1).where('user_id', args.uid)
    );
  }

  return Promise.all(promiseArray).then(([x, sender, invite]) =>
    send_mail(args.mailList, sender[0], invite.invite_id, args.category)
  );
}; //exports
