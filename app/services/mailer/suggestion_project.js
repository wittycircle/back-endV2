...
const project_bloc = (picture, title, description, location, project_status, need_status, skills, public_id) => {
	const noLogo = 'https://res.cloudinary.com/dqpkpmrgk/image/upload/v1501823817/Witty-icon/Icon_NoLogo_Project.png';

	return `<div class="main-class"><div class="bloc1" style="display: inline-block;"><div class="first-container" style="margin-bottom: 10px;"><img style="border-radius: 4px; width: 50px; height: 50px;" src="${picture || noLogo}" alt="project_picture">
</div><div class="second-container" style="margin: 0 10px 10px 10px;"><h2 class="sc-1">${title}</h2><h3 class="space-normal sc-2">${description || ''}</h3><div class="location" style="margin-bottom: 10px;">${wm.location_bloc(wm.location(location))}
...
</div><div class="skill-list space-normal"><p class="sl-p">Looking for ${transformStatus(need_status)}</p><ul class="noPadding noMargin">${wm.skills_bloc2(skills)}</ul></div></div></div>
...
const saveSentData = (projects, profile) => {
	...
	db(TABLES.SUG_PROJECTS).insert(data).then(r => { return null });
}
