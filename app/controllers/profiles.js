...
exports.updateProfile = (req, res, next) => {
	...
	profiles
		.updateProfile(req.body, location, { user_id: req.user.id })
		.then(r => {
			if (r) {
				req.broadcastEvent('profile_update', { id: req.user.id });
...
exports.autoFollowProfile = (req, user_id, follow_id) => {
	return profiles
		.followProfile(follow_id, user_id)
		.then(r => {
			// if (_.isEmpty(r)) {
			...
			//     value: -1
			//   });
			//   req.broadcastEvent('add_points', { user_id: follow_id, points: -2 });
			//   req.broadcastEvent('add_points', { user_id: user_id, points: -1 });
