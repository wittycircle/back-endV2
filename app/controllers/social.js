...
const addExperienceToUser = id => experiences => experiences
    .map(([experience, location]) => users.addExperience(id, experience, location));
const addSkillsToUser = id => skills => Promise.all(
    skills.map(skill => addSkill(_.lowerCase(skill)))
);
...
exports.updateProfileFromLinkedin = (req, res, next) => {
    ...
    social.getLinkedinProfileInfo(profile)
        .then(({skills, summary, experiences}) => Promise.all([
                        ...
                        return [
                            {
                                ...
                                date_to:   _.lowerCase(date_to) === 'present' ? Date.now() : Date.parse(date_to),
                                description
                            },
                            {
                                city,
...
exports.InviteFriendsFromGoogle = (req, res, next) => {
    ...
    social.gmailContactsCampaign(token)
        .then(mailList => mailer.invite_user({uid: req.user, mailList, category: 'gmail'}))
