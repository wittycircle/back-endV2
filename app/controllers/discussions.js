...
exports.removeProjectDiscussion = (req, res, next) => {
  discussion
    .removeProjectDiscussion(req.params.discussion_id)
    .then(r => {
      if (typeof r === 'string') {
        return next([r, r]);
      } else {
...
exports.getDiscussionReplies = (req, res, next) => {
  discussion.getDiscussionReplies(req.params.discussion_id).then(r => {
    if (typeof r === 'string') {
      return next([r, 'Bad id']);
    } else {
      if (req.user) {
...
exports.replyDiscussion = (req, res, next) => {
  ...
  discussion
    .replyDiscussion(req.params.discussion_id, req.user.id, req.body.message)
    .then(r => {
      if (typeof r === 'string') {
...
exports.likeDiscussion = (req, res, next) => {
  discussion
    .likeDiscussion(req.params.discussion_id, req.user.id)
    .then(r => {
      if (typeof r === 'string') {
...
exports.unlikeDiscussion = (req, res, next) => {
  discussion
    .unlikeDiscussion(req.params.discussion_id, req.user.id)
    .then(r => {
      if (typeof r === 'string') {
