...
exports.getProject = (id) => {
    ...
    return req.then((r) => {
        r.forEach(el => {
            x.push(exports.getProjectDiscussion(el.id).then(rr => {
                el.discussions = rr
            }));
            x.push(exports.getProjectOpenings(el.id).then(rr => {
                el.openings = rr
            }));
            x.push(getMembers(el.id).then(rr => el.members = rr));
            x.push(getFollowerCount(el.id).then(rr => {
