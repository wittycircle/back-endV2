...
exports.getProfileBy = by => {
  const ifo = db
    .distinct('follow_user_id', 'user_id')
  ...
  return h.exist(TABLES.USER_PROFILES, by['p.id']).then(() => query);
};

exports.updateProfile = (stuff, cnd) => {
  return db(TABLES.USER_PROFILES).update(stuff).where(cnd);
};
// ------------------ Follow ------------------
exports.getProfileFollowers = (cond, cond2, p_id) => {
  return h.exist(TABLES.USERS, p_id).then(r => {
