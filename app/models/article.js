...
const removeOldTag = (article_id) => {
	return db(TABLES.TAG_ARTICLES)
		.del()
		.where('article_id', article_id)
};

const updateTags = (article_id, data) => {
		...
		.then(r => {
			r.forEach(e => {
				db(TABLES.TAG_ARTICLES)
					.insert({ article_id: article_id, tag_id: e.id})
					.return()
			});
			return ;
		})
...
exports.updateArticle = (data, id) => {
	...
	if (data.picture) i_data.picture 		= data.picture;
	if (data.read_time) i_data.read_time 	= data.read_time
	...
	return h.admin(TABLES.ARTICLES, id, data.uid).then(([r, r1]) => {
		if (!r.length || !r1.length || !data.tags) return 'Not an admin';
			...
			return db(TABLES.ARTICLES)
				.update(i_data)
				.where('id', id)
				.then(removeOldTag(id))
				.then(res => {
					updateTags(id, data.tags);
